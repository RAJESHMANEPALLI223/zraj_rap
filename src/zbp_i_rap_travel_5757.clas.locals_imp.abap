...
CLASS lhc_travel IMPLEMENTATION.
 ...
 "   READ ENTITIES OF zi_rap_travel_5757 IN LOCAL MODE
    "  ENTITY Travel
  ...
  " Set the new overall status
    MODIFY ENTITIES OF zi_rap_travel_5757 IN LOCAL MODE
      ENTITY Travel
         UPDATE
           FIELDS ( TravelStatus )
  ...
  METHOD calculateTravelID.
   ...
   " Please note that this is just an example for calculating a field during
"_onSave_.
    " This approach does NOT ensure for gap free or unique travel IDs! It just helps
"to provide a readable ID.
  ...
  METHOD validateAgency.
   ...
   RESULT DATA(travels).
    ...
    agencies = CORRESPONDING #( travels DISCARDING DUPLICATES MAPPING agency_id =
AgencyID EXCEPT * ).
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id =
travel-AgencyID ] ).
        APPEND VALUE #( %tky = travel-%tky ) TO failed-travel.

        APPEND VALUE #( %tky        = travel-%tky
                        %state_area = 'VALIDATE_AGENCY'
                        %msg        = NEW zcm_rap_5757(
                                          severity = if_abap_behv_message=>severity-error
                                          textid   = zcm_rap_5757=>agency_unknown
  ...
  METHOD validateCustomer.
    ...
    customers = CORRESPONDING #( travels DISCARDING DUPLICATES MAPPING customer_id =
CustomerID EXCEPT * ).
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id =
travel-CustomerID ] ).
        APPEND VALUE #(  %tky = travel-%tky ) TO failed-travel.
...
if_abap_behv_message=>severity-error
                                                 textid    =
zcm_rap_5757=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
...
CLASS lhc_Booking IMPLEMENTATION.
  ...
  DATA max_bookingid TYPE /dmo/booking_id.
    DATA update TYPE TABLE FOR UPDATE ZI_RAP_Travel_5757\\Booking.

    " Read all travels for the requested bookings.
    " If multiple bookings of the same travel are requested, the travel is returned
"only once.
    READ ENTITIES OF ZI_RAP_Travel_5757 IN LOCAL MODE
    ENTITY Booking BY \_Travel
      ...
      " Process all affected Travels. Read respective bookings, determine the max-id
